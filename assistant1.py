...
class Widget:
    def __init__(self):
        ...
        userFrame = LabelFrame(root, text='AyruS', font=('plexifont', 24, 'bold'))
        ...
        Button(root, text='Close', font=('railways', 10, 'bold'), bg='yellow', fg='black', command=root.destroy).pack(
            fill='x', expand='no')
        lee_voice('How can i help you Surya?')
        root.mainloop()
    ...
    def clicked(self):
        ...
        if 'who are you' in voice_data:
            lee_voice('My name is AyruS ')
        if 'search' in voice_data:
            search = record_audio('What do you want to search for ?')
            url = 'https://google.com/search?q=' + search
            webbrowser.get().open(url)
            lee_voice('Here is what i found about ' + search)
        if 'play' in voice_data:
            song = record_audio('what do you like to watch?')
        ...
        if "open blender" in voice_data:
            lee_voice("Opening pink hole")
            os.startfile('pink hole.blend')
